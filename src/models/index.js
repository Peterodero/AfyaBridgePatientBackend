...
const RefreshToken = sequelize.define('RefreshToken', {
  ...
  patientId: { type: DataTypes.STRING(30), allowNull: false },
...
const SymptomMessage = sequelize.define('SymptomMessage', {
  ...
  sessionId: { type: DataTypes.STRING(30), allowNull: false },
...
const Specialist = sequelize.define('Specialist', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `DOC-${Date.now()}` },
  name: { type: DataTypes.STRING(100), allowNull: false },
...
const AppointmentSlot = sequelize.define('AppointmentSlot', {
  id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
  specialistId: { type: DataTypes.STRING(30), allowNull: false },
...
const Appointment = sequelize.define('Appointment', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `APT-${Date.now()}` },
  patientId: { type: DataTypes.STRING(30), allowNull: false },
  specialistId: { type: DataTypes.STRING(30), allowNull: false },
...
const Consultation = sequelize.define('Consultation', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `CON-${Date.now()}` },
  appointmentId: { type: DataTypes.STRING(30), allowNull: false },
...
const ConsultationMessage = sequelize.define('ConsultationMessage', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `CHAT-${Date.now()}` },
  consultationId: { type: DataTypes.STRING(30), allowNull: false },
...
const Prescription = sequelize.define('Prescription', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `PRX-${Date.now()}` },
  patientId: { type: DataTypes.STRING(30), allowNull: false },
  specialistId: DataTypes.STRING(30),
  appointmentId: DataTypes.STRING(30),
  name: { type: DataTypes.STRING(150), allowNull: false },
  dosage: DataTypes.STRING(100),
...
const Pharmacy = sequelize.define('Pharmacy', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `PH-${Date.now()}` },
  name: { type: DataTypes.STRING(100), allowNull: false },
...
const RefillOrder = sequelize.define('RefillOrder', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `REF-${Date.now()}` },
  patientId: { type: DataTypes.STRING(30), allowNull: false },
  pharmacyId: DataTypes.STRING(30),
  ...
  status: {
    type: DataTypes.ENUM('pending', 'processing', 'ready', 'out_for_delivery', 'delivered', 'cancelled'),
...
const RefillOrderItem = sequelize.define('RefillOrderItem', {
  id: { type: DataTypes.INTEGER, autoIncrement: true, primaryKey: true },
  refillOrderId: { type: DataTypes.STRING(30), allowNull: false },
  prescriptionId: { type: DataTypes.STRING(30), allowNull: false },
  quantity: { type: DataTypes.INTEGER, defaultValue: 1 },
  price: DataTypes.DECIMAL(10, 2),
});
...
const Payment = sequelize.define('Payment', {
  id: { type: DataTypes.STRING(30), primaryKey: true, defaultValue: () => `TXN-${Date.now()}` },
  patientId: { type: DataTypes.STRING(30), allowNull: false },
  refillOrderId: DataTypes.STRING(30),
  appointmentId: DataTypes.STRING(30),
  type: { type: DataTypes.ENUM('appointment', 'refill'), defaultValue: 'refill' },
  ...
  checkoutRequestId: DataTypes.STRING(100), // Daraja STK push ID — stored so callback can find the right payment
...
const Delivery = sequelize.define('Delivery', {
  ...
  status: {
    type: DataTypes.ENUM('assigned', 'picked_up', 'out_for_delivery', 'delivered'),
...
Specialist.hasMany(Appointment, { foreignKey: 'specialistId' });

// AppointmentSlot
AppointmentSlot.belongsTo(Specialist, { foreignKey: 'specialistId' });
Specialist.hasMany(AppointmentSlot, { foreignKey: 'specialistId' });

// Consultation
Consultation.belongsTo(Appointment, { foreignKey: 'appointmentId' });
Appointment.hasOne(Consultation, { foreignKey: 'appointmentId' });
ConsultationMessage.belongsTo(Consultation, { foreignKey: 'consultationId' });
Consultation.hasMany(ConsultationMessage, { foreignKey: 'consultationId' });

// Prescription
Prescription.belongsTo(Patient, { foreignKey: 'patientId' });
Prescription.belongsTo(Specialist, { foreignKey: 'specialistId' });
Prescription.belongsTo(Appointment, { foreignKey: 'appointmentId' });
Patient.hasMany(Prescription, { foreignKey: 'patientId' });

// RefillOrder
RefillOrder.belongsTo(Patient, { foreignKey: 'patientId' });
RefillOrder.belongsTo(Pharmacy, { foreignKey: 'pharmacyId' });
RefillOrder.hasMany(RefillOrderItem, { foreignKey: 'refillOrderId' });
RefillOrderItem.belongsTo(RefillOrder, { foreignKey: 'refillOrderId' });
RefillOrderItem.belongsTo(Prescription, { foreignKey: 'prescriptionId' });

// Payment
Payment.belongsTo(Patient, { foreignKey: 'patientId' });
Payment.belongsTo(RefillOrder, { foreignKey: 'refillOrderId' });
Payment.belongsTo(Appointment, { foreignKey: 'appointmentId' });

// Delivery
Delivery.belongsTo(RefillOrder, { foreignKey: 'refillOrderId' });
RefillOrder.hasOne(Delivery, { foreignKey: 'refillOrderId' });
...
module.exports = {
  ...
  // Doctor
  Specialist, AppointmentSlot, Appointment, Consultation, ConsultationMessage, Prescription,
  // Pharmacy
  Pharmacy, RefillOrder, RefillOrderItem,
  // Payment & Delivery (shared)
  Payment, Delivery,
};